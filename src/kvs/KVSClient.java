...
public class KVSClient implements KVS {
  ...
  public void batchPut(String tableName, List<Row> rows) throws IOException {
    rows.stream()
            .mapToInt(row -> workerIndexForKey(row.key()))
            .distinct()
            .forEach(workerIndex -> {
                // Filter rows for the current worker index
                List<Row> rowsForWorker = rows.stream()
                                              .filter(row -> workerIndexForKey(row.key()) == workerIndex)
                                              .collect(Collectors.toList());

                // Convert rows to byte arrays and concatenate into a single byte array
                byte[] bigByteArray = rowsForWorker.stream()
                                                    .map(Row::toByteArray)
                ...
                // Perform batch PUT request to the current worker index
                try {
                  byte[] response = HTTP.doRequest("PUT", "http://" + workers.get(workerIndex).address + "/data/" + tableName, bigByteArray).body();
                String result = new String(response);
                } catch(IOException e) {
                  e.printStackTrace();
                }              
  ...
  public void post(String tableName, String row, String column, byte value[]) throws IOException {
    if (!haveWorkers)
      downloadWorkers();
    ...
    try {
      ...
      byte[] response = HTTP.doRequest("POST", target, value).body();
      String result = new String(response);
      if (!result.equals("OK"))
        throw new RuntimeException("POST returned something other than OK: "+result+ "("+target+")");
    } catch (UnsupportedEncodingException uee) {
      throw new RuntimeException("UTF-8 encoding not supported?!?");
    }
  }
  
  public void post(String tableName, String row, String column, String value) throws IOException {
  ...
  public void putRow(String tableName, Row row) throws FileNotFoundException, IOException {
    ...
    String s = "http://"+workers.elementAt(workerIndexForKey(row.key())).address+"/data/"+tableName;
  ...
  public int count(String tableName) throws IOException {
    ...
    for (WorkerEntry w : workers) {
      ...
      if ((r != null) && (r.statusCode() == 200)) {
        ...
        if(result.equals("webserve")) return 0;
